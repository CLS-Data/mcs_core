#DACAQ_P:	DV NVQ equivalent of highest Academic qualification across previous sweeps (Partner Interviewee)
...
attr(dacaq_dnvq_all$DNVQ_P, "label") <- "DV Respondent NVQ Highest Level (all sweeps- Partner Interviewee)"
table(dacaq_dnvq_all$DNVQ_P, dacaq_dnvq_all$SWEEP, useNA = "ifany")

dacaq_dnvq_all <- dacaq_dnvq_all %>%
  mutate(DACAQ_P = case_when(
    SWEEP == 1 ~ ADACAQ00,
    SWEEP == 2 ~ BDACAQ00,
    SWEEP == 3 ~ CDACAQ00,
    SWEEP == 4 ~ DDACAQ00,
...
dacaq_dnvq_all <- dacaq_dnvq_all %>% select(MCSID, SWEEP, DACAQ_P, DNVQ_P)
dacaq_dnvq_all <- dacaq_dnvq_all  %>%
  mutate(SWEEP = labelled(SWEEP,labels = c( "MCS1" = 1, "MCS2" = 2, "MCS3" = 3, "MCS4" = 4, "MCS5" = 5, "MCS6" = 6)))
attr(dacaq_dnvq_all$SWEEP, "label") <- "MCS Sweep"
saveRDS(dacaq_dnvq_all, file = file.path(temp_data_pdv, "DACAQ_DNVQ_P.Rds"))
